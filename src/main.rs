...
fn main() {
    ...
    let mut players: HashMap<_, _> = vec![Player::blue(), Player::red()].into_iter().map(|p|(p.id, p)).collect();
    ...
    for tile in deck {
        ...
        let player_id = player_id_iter.next().expect("should always have a next player while tiles remain");
        ...
        if let Some(random_move) = selected_move_hint {
            let tile = PlacedTile {
                ...
                meeple: if let (Some(region_index), Some(meeple)) = (random_move.meeple_placement, player.meeple.pop()) {
